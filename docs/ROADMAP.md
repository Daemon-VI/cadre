...
| **D3** | GitHub Action (project mode on the checkout → PR; trusted triggers only) | FR-18 | **verified 2026-09-19** on `Daemon-VI/cadre-action-demo`: the third run opened PR #2 (18 calls, all 7 tests pass on the branch); the first two runs found four bugs, all fixed; not listed on the Marketplace; no `v1` tag yet |
