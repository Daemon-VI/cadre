...
> **Not yet published.** The image and the standalone builds are produced by the release workflow,
> which has not run yet, so neither `ghcr.io/daemon-vi/cadre` nor a GitHub Release exists today.
> The instructions below describe what that workflow builds.
